<div class="container">
	<div class="collection">
		 <a href="?controller=Menu2&action=CargarOpcion&opcion=CrearUsuario" class="collection-item black-text"><b>Crear Usuario</b></a>
		 <a href="#!" class="collection-item black-text"><b>Ver Usuario</b></a>
		 <a href="#!" class="collection-item black-text"><b> Inventario de Libros </b></a>
		 <a href="?controller=Menu3&action=CargarOpcion&opcion=MostrarP" class="collection-item black-text"><b>Inventario Uniforme</b></a>
		 <a href="#!" class="collection-item black-text"><b>Agregar Libro</b></a>
		 <a href="#!" class="collection-item black-text"><b> Agregar Uniformes </b></a>
		 <a href="#!" class="collection-item black-text"><b>Quitar Libro </b></a>
		 <a href="#!" class="collection-item black-text"><b>Quitar Uniforme</b></a>
  </div>
</div>